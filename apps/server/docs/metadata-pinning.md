...
| `FailingPinner`   | production-like config (including chain 4663) without `PINATA_JWT` | never returns - throws (fail closed)                             |
...
- Pin the six tier images (`/assets/tiers/<tier>.svg`) and point
  `METADATA_BASE_URL` at the public gateway so `/api/config` consumers and
  wallets can resolve images.
- Enable production mode (`NODE_ENV=production`) with KMS config - or use
  chain 4663, which is always production-like - and set `PINATA_JWT`; otherwise
  every mint stays delayed.
